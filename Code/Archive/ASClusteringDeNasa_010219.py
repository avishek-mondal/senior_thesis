...
def clustering(sizes, similarity_index_name, date, ASSimilarityFileName):
	cluster_sizes = []
	...
	for num_clusters in cluster_sizes:
		...
		cluster_array = {}
		with open(ASSimilarityFileName) as data_file:
			...
			first_AS_cluster_rep, first_AS_cluster_relationships = random.choice(list(as_list.items()))
		...
		data_file.close()
		outputFileName = '../Data/similarity_clusters_' + str(num_clusters) + '_'+ similarity_index_name+'_'+date+ '.json'
...
def clusterRepsRandom(num_clusters, cluster_reps_array):
	for i in range(num_clusters):
		#TODO
		new_cluster_rep, _ = random.choice(list(as_list.items()))
		cluster_reps_array[new_cluster_rep] = as_listCopy[new_cluster_rep]
		as_list.pop(new_cluster_rep)


def clustering_test(sizes, similarity_index_name, date, ASSimilarityFileName):
	cluster_sizes = []
	...
	for num_clusters in cluster_sizes:
		...
		with open(ASSimilarityFileName) as data_file:
			...
			for key, values in data.items():
				as_list[key] = values
				as_listCopy[key] = values
			#pick random cluster_representatives
			clusterRepsRandom(num_clusters, cluster_reps_array)
